...
export function createClient(userId) {
  ...
  if (!execPath) {
    try {
      execPath = execSync('which chromium || which chromium-browser || which google-chrome').toString().trim();
      console.log(`[WhatsApp] Found system browser at: ${execPath}`)
    } catch (e) {
      console.log(`[WhatsApp] System browser not found, using bundled Chromium`)
      execPath = undefined;
    }
  }
  ...
  client.initialize().catch(err => {
    ...
    console.error(`[WhatsApp] ERROR DETAILS: ${err.message}\n${err.stack}`)
    clients.delete(userId)
    qrCodes.delete(userId)
    updateSessionStatus(userId, 'disconnected')
  })
